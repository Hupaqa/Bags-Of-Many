...
function OnPlayerSpawned( player_entity ) -- This runs when player entity has been created
    add_item_to_inventory(player_entity, "mods/bags_of_many/files/entities/bags/bag_small.xml")
    add_item_to_inventory(player_entity, "mods/bags_of_many/files/entities/bags/bag_small.xml")
end
...
function setup_gui()
    ...
    if inventory_open and GuiImageButton(gui, new_id(), button_pos_x, button_pos_y, "", "mods/bags_of_many/files/ui_gfx/material_backpack.png") then
		open = not open
    ...
    if inventory_open and is_bag(active_item) then
        ...
        for i = 1, nb_of_items do
            ...
            if stored_items[i] ~= nil then
                local sprite_component = EntityGetComponentIncludingDisabled(stored_items[i], "SpriteComponent")
                if sprite_component then
                    local sprite_path = ComponentGetValue2(sprite_component[1], "image_file")
                    if sprite_path then
